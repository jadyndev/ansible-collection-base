...
password_slug:
    description:
        - Returns the password slug
    type: str
    returned: always
diff:
    description:
        - List of the old and current gpg recipients key-ids
    type: dict
    returned: changed
message:
    description:
        - Human-readable information about the (completed) task
    type: str
    returned: changed
warning:
...
try:
    ...
    HAS_LIB = True
    HAS_LIB_FILELOCK = True
except ImportError:
    HAS_LIB_FILELOCK = False
...
def main():
    ...
    if not HAS_LIB_FILELOCK:
        module.fail_json(msg=missing_required_lib("filelock"))
    if not HAS_LIB_GIT:
        module.fail_json(msg=missing_required_lib("GitPython"))
    if not HAS_LIB_GNUPG:
        module.fail_json(msg=missing_required_lib("python-gnupg"))
    if not HAS_LIB_YAML:
